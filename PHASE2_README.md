...
> **Headline decision (read this first):** The most valuable thing in Phase 2 is **NOT multi-camera for its own sake** — it's **WMS integration**. Multi-camera gives you _coverage_; WMS integration gives you _meaning_. "23 people in Picking" becomes "Picking is 15% understaffed for tonight's order volume." We build both, but WMS is the value headline. Robots stay in Phase 3 — see §12.
...
### 1.1 Rendered diagram (Mermaid) — edge fleet + backend microservices

```mermaid
flowchart TD
    subgraph EDGEFLEET["EDGE FLEET — ~5 GPU boxes · 50+ cameras (raw video NEVER leaves site)"]
        direction LR
        ...
        EN["edge box N ...<br/>same pipeline + SQLite spool"]
    end

    E1 & E2 & EN ==>|"track events + ReID embeddings · mTLS<br/>events only, never raw video"| INGEST

    subgraph SERVER["SERVER TIER — Kubernetes (no GPU*) · scalable services"]
        direction TB

        subgraph BACKEND["Backend microservices"]
            direction TB
            INGEST["ingest-api · FastAPI<br/>validate + auth (mTLS)"]
            KAFKA["kafka / redpanda<br/>durable event bus · backpressure"]
            REID["reid-stitcher<br/>camera adjacency graph<br/>spatial-temporal + appearance"]
            WMSC["wms-connector<br/>orders · tasks · inventory · labor"]
            DERIVE["deriver (LIVE) + replayer (BATCH)<br/>same code · events + WMS + config"]
            API["api · FastAPI<br/>REST + WebSocket"]
            PLAY["playback-svc<br/>reconstruct past twin state"]
        end
        ...
        subgraph DATA["Data stores"]
            ...
            METRIC[("Metrics · Timescale<br/>occupancy · dwell · flow")]
            HEAT[("Heatmaps<br/>floor-plan density")]
            SAFE[("Safety / PPE<br/>+ Restricted Zone")]
            DOCK[("Dock / Yard state")]
            PLAN[("Labor-vs-Plan")]
        end

        CFG[/"Zone / Rule Config<br/>versioned JSON"/]
        DASH["dashboard · Next.js<br/>warehouse-wide map · staffing-vs-plan<br/>safety · dock · historical scrubber"]
        OBS["prometheus + grafana<br/>per-cam health · GPU · reid match rate · SLO"]
    ...
    EDGEFLEET -.metrics.-> OBS

    %% * Default: ReID embeddings computed on edge, matching on CPU server-side. Small server GPU only if matching is moved server-side.
```

### 1.2 ASCII version (fallback for non-Mermaid viewers)
...
1. **Spatial-temporal handoff (primary, cheap, reliable).** Cameras are calibrated to a _shared floor plan_ (Phase 1 homography). When a track exits camera A's field of view at world point `P` and a new track appears in camera B near `P` within `Δt`, they're the same object. A **camera adjacency graph** (which cameras' FOVs touch/overlap) constrains matching to neighbours only.
...
**Why this works in a warehouse specifically:** fixed cameras, known overlapping coverage, constrained movement along aisles → spatial-temporal matching does ~80% of the work. Appearance only breaks ties. This is far more reliable than appearance-only re-ID (which fails badly on people in identical hi-vis vests — and _everyone_ in a warehouse wears the same vest).

**Honest limitation:** coverage gaps (un-cameraed corridors) create "teleport" ambiguity. Mitigate with a _gap-transit model_ (expected travel time between adjacent FOVs) and accept that identity may reset across large blind spots. We track _flows and counts_ reliably; perfect lifelong per-person paths are a stretch goal, not a promise.
...
- **Tasks / pick waves** — who _should_ be doing what, where, by when.
...
**Join key = zone + time.** The twin already knows _actual_ occupancy/activity per zone per minute (anonymous). The WMS knows _planned_ work per zone. Joining them produces the metrics managers actually act on:
...
```json
{
  ...
  "staffing_gap": -6,
  "open_pick_tasks": 312,
  "throughput_picks_per_hr": 410,
  "required_rate": 520,
  "status": "behind_plan",
  "projected_miss_minutes": 35
}
```

**Privacy note:** the twin stays **anonymous** (zone counts, not named individuals). The WMS supplies the _work_ context; we never join a camera track to a named employee. Keep that boundary explicit — it's what makes the safety/labor story defensible.
...
Because the raw event log is append-only and complete, both are _replays_:

- **Historical analytics** = run the same Deriver over a past time range → trends, shift comparisons, zone utilization over weeks.
- **Event playback** = reconstruct twin state at any past timestamp and scrub forward (the dashboard reads historical twin snapshots instead of live). No video needed — we replay _events_, which is why this is cheap and privacy-safe.
...
Naive: 50 cams × 10 objects × 5 fps = **2,500 track events/sec**. At 5 fps the position stream is wasteful for the _log_.
...
**Overall verdict: YES, it works — on ~5 edge boxes + one modest server VM + Kafka + TimescaleDB.** The binding constraints are _not_ compute or network. They are (in order): **(1) cross-camera re-ID accuracy, (2) WMS data quality, (3) calibrating 50 homographies.** All three are _engineering/data_ problems, not capacity walls.
...
| -------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------- |
| **Shift supervisor**       | Walks the floor, eyeballs where people are, reacts       | Sees live staffing-vs-plan per zone; rebalances _before_ a zone falls behind     | Twin actual-occupancy joined to WMS planned-labor |
...
**The core shift:** from **reactive + anecdotal** to **proactive + quantified**. The twin doesn't replace people — it gives every role a _minutes-ahead_ view instead of an _after-the-fact_ one.
...
**Why the value compounds vs Phase 1:** Phase 1 told you _what is happening_. Phase 2 tells you _whether what's happening matches the plan, across the whole building, and what it's costing you_ — that's the gap between a dashboard people glance at and a system the operation runs on.

**Example back-of-envelope:** a site with 200 floor staff at $20/hr, 2 shifts, ~$16M/yr labor. A _conservative 4%_ productivity gain ≈ **$640k/yr** — against ~5 edge boxes (~$15–20k hardware) + a server + integration effort. Payback is measured in **weeks**, not years, _if_ the labor-rebalancing lever is real for this operation. **Validate that lever first** — it's where most of the ROI sits.

---

## 9. Deployment & ops at scale

- **Edge:** still Docker Compose _per box_, but now **fleet-managed** (Ansible or Balena/Portainer) — config, model engine, and zone files pushed centrally. Each box pinned to image tags; staged rollout.
...
| -------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Cross-camera re-ID accuracy** (identical vests, blind spots) | **High** | Spatial-temporal first, appearance second; adjacency graph; provisional gating; promise _flows/counts_, not perfect lifelong paths |
...
AMRs self-localize (LiDAR/SLAM) more precisely than CCTV ever will, and they already have a fleet manager. Drawing robots on the twin adds little. The _real_ robot value — using the twin's unique **human/zone truth** to keep AMRs and people safely orchestrated — requires a **mature, trusted, accurate** twin first. Phase 2's job is to _earn that trust_. Connect robots in Phase 3, once the twin is the authoritative source of human/zone state a robot would be willing to listen to.
